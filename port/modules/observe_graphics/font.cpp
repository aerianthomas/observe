...
namespace observe
{
  ...
  public:
    ...
    {
      OBSERVE_ASSERT(instance().faces_.count(face));
  ...
  public:
    ...
    PackedFont(const std::string &id, int size, const unsigned char *data, int data_size) : id_(id), size_(size), data_size_(data_size)
    {
  ...
  Font::Font(float size, const unsigned char *font_data, int data_size, float dpi_scale) : size_(size), dpi_scale_(dpi_scale)
  {
  ...
  Font::Font(float size, const EmbeddedFile &file, float dpi_scale) : size_(size), dpi_scale_(dpi_scale)
  {
    native_size_ = std::round(size * (dpi_scale ? dpi_scale : 1.0f));
    packed_font_ = FontCache::loadPackedFont(native_size_, file);
  }

  Font::Font(float size, const std::string &file_path, float dpi_scale) : size_(size), dpi_scale_(dpi_scale)
  {
    native_size_ = std::round(size * (dpi_scale ? dpi_scale : 1.0f));
  ...
  {
    OBSERVE_ASSERT(Thread::isMainThread());
  ...
  void FontCache::decrementPackedFont(PackedFont *packed_font)
  {
    ...
    OBSERVE_ASSERT(ref_count_[packed_font] >= 0);
  }
