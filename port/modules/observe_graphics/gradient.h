...
namespace observe
{
  ...
  public:
    ...
    {
      auto sample_function = [&](float s)
      { return from.sample(s).interpolateWith(to.sample(s), t); };
    ...
    {
      OBSERVE_ASSERT(index < colors_.size());
      colors_[index] = color;
    }
    ...
    {
      auto sample_function = [&](float s)
      { return sample(s) * other.sample(s); };
  ...
  private:
    void sort()
    {
      ...
      std::sort(indices.begin(), indices.end(),
                [&](int a, int b)
                { return Color::compare(colors_[a], colors_[b]) < 0; });
  ...
  public:
    ...
    struct PackedGradientReference
    {
      PackedGradientReference(std::weak_ptr<GradientAtlas *> atlas,
                              const PackedGradientRect *packed_gradient_rect) : atlas(std::move(atlas)), packed_gradient_rect(packed_gradient_rect) {}
    ...
    public:
      int x() const
      {
        OBSERVE_ASSERT(reference_->atlas.lock().get());
      ...
      explicit PackedGradient(std::shared_ptr<PackedGradientReference> reference) : reference_(std::move(reference)) {}
  ...
  private:
    ...
    {
      OBSERVE_ASSERT(gradients_.count(gradient));
    ...
    OBSERVE_LEAK_CHECKER(GradientAtlas)
  };
  ...
  struct GradientPosition
  {
    ...
    {
      OBSERVE_ASSERT(from.shape == to.shape || from.shape == InterpolationShape::Solid ||
    ...
    GradientPosition(Point from, Point to) : shape(InterpolationShape::PointsLinear), point1(from), point2(to) {}
    ...
    OBSERVE_LEAK_CHECKER(GradientPosition)
  };
  ...
  public:
    ...
    Brush(Gradient gradient, const GradientPosition &position) : gradient_(std::move(gradient)), position_(position) {}
  ...
  public:
    ...
    PackedBrush(GradientAtlas *atlas, const Gradient &gradient, const GradientPosition &position) : atlas_(atlas), position_(position), gradient_(atlas->addGradient(gradient)) {}

    PackedBrush(GradientAtlas *atlas, const Brush &brush) : atlas_(atlas), position_(brush.position()), gradient_(atlas->addGradient(brush.gradient())) {}
  ...
  private:
    ...
    OBSERVE_LEAK_CHECKER(PackedBrush)
  };
}