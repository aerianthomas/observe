...
namespace observe
{
  ...
  {
    Image() = default;
    Image(const unsigned char *data, int data_size, int width = 0, int height = 0) : data(data), data_size(data_size), width(width), height(height) {}
  ...
  public:
    ...
    {
      OBSERVE_ASSERT(index >= 0 && index < num_points_);
  ...
  {
  public:
    HeatMapData(int width = 0, int height = 0) : width_(width), height_(height), values_(width * height, 0.0f) {}
  ...
  public:
    ...
    {
      PackedImageReference(std::weak_ptr<ImageAtlas *> atlas, const PackedImageRect *packed_image_rect) : atlas(std::move(atlas)), packed_image_rect(packed_image_rect) {}
    ...
    public:
      ...
      explicit PackedImage(std::shared_ptr<PackedImageReference> reference) : reference_(std::move(reference)) {}
  ...
  private:
    ...
    {
      OBSERVE_ASSERT(images_.count(image));
