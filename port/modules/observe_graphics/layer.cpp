...
namespace observe
{
  ...
  {
    RegionPosition(Region *region, std::vector<IBounds> invalid_rects, int position, int x = 0, int y = 0) : region(region), invalid_rects(std::move(invalid_rects)), position(position), x(x), y(y) {}
    ...
    for (Region *region : regions_)
    {
      if (region->backdropEffect())
      {
        ...
        if (std::any_of(top_level_invalid_rects.begin(), top_level_invalid_rects.end(),
                        [&bounds](const auto &invalid_rect)
                        { return bounds.overlaps(invalid_rect); }))
        {
          region->invalidate();
        }
      }
