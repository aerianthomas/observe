...
namespace observe
{
  ...
  void Path::loadCommands(const CommandList &commands)
  {
    ...
    for (const auto &command : commands)
    {
      ...
      case 'L':
        ...
        break;
      case 'H':
        horizontalTo(command.end.x);
        break;
      case 'V':
        verticalTo(command.end.y);
        break;
      case 'Q':
        quadraticTo(command.control1, command.end);
        break;
      case 'T':
        smoothQuadraticTo(command.end);
        break;
      case 'C':
        bezierTo(command.control1, command.control2, command.end);
        break;
      case 'S':
        smoothBezierTo(command.control1, command.end);
        break;
      case 'A':
    ...
    for (const auto &sub_path : paths_)
    {
      ...
      for (const Point &next : sub_path.points)
      {
        ...
        else if (type == Join::Square)
        {
          ...
          OBSERVE_ASSERT(intersection_prev.has_value());
          OBSERVE_ASSERT(intersection.has_value());
  ...
  int PathAtlas::updatePaths(int submit_pass)
  {
    ...
    {
      OBSERVE_LOG("PathAtlas::updatePaths: Failed to allocate transient buffers");
    ...
    {
      OBSERVE_LOG("PathAtlas::updatePaths: Failed to map transient buffers");
      return submit_pass + 1;
    }
    ...
    OBSERVE_ASSERT(vertex == num_vertices);
    OBSERVE_ASSERT(triangle_index == num_indices);
