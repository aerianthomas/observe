...
namespace observe
{
  ...
  public:
    ...
    {
      PackedPathReference(std::weak_ptr<PathAtlas *> atlas, const PackedPathRect *packed_path_rect) : atlas(std::move(atlas)), packed_path_rect(packed_path_rect) {}
      ~PackedPathReference();
    ...
    {
    public:
      explicit PackedPath(std::shared_ptr<PackedPathReference> reference) : reference_(std::move(reference)) {}
  ...
  private:
    ...
    OBSERVE_LEAK_CHECKER(PathAtlas)
  };
}
