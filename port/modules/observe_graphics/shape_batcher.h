...
namespace observe
{
  ...
  QuadVertices<typename T::Vertex> setupQuads(const BatchVector<T> &batches)
  {
    ...
    OBSERVE_ASSERT(vertex_index == results.num_shapes * kVerticesPerQuad);
    return results;
  }
  ...
  public:
    ...
    {
      OBSERVE_ASSERT(id_ == nullptr || id_ == shape.batch_id);
      id_ = shape.batch_id;
  ...
  public:
    ...
    void submit(Layer &layer, int submit_pass, const std::vector<PositionedBatch> &batches) override
    {
      ...
      {
        OBSERVE_ASSERT(batch.batch->id() == id());
  ...
  public:
    ...
    void addShape(T shape, BlendMode blend = BlendMode::Alpha)
    {
      ...
      ShapeBatch<T> *batch = match ? reinterpret_cast<ShapeBatch<T> *>(batches_[batch_index].get()) : createNewBatch<T>(shape.batch_id, blend, batch_index);
