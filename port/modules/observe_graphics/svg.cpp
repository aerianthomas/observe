...
namespace observe
{
  ...
  {
    constexpr auto is_whitespace = [](char c)
  ...
  std::pair<std::string, std::string> parseAttribute(const std::string &str, int &i)
  {
    ...
    OBSERVE_ASSERT(quote_char == '"' || quote_char == '\'');
  ...
  TagData parseTag(const std::string &str, int &i)
  {
    ...
    {
      OBSERVE_ASSERT(!tag_data.name.empty() && i < str.size() && str[i] == '>');
      return tag_data;
    }
  ...
  Tag parseTagTree(const std::string &str, int &i)
  {
    ...
    OBSERVE_ASSERT(tag.data.name == child.data.name);
    return tag;
  }
