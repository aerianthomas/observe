...
namespace observe
{
  ...
  {
  public:
    ScrollBar() : color_(Animation<float>::kRegularTime, Animation<float>::kEaseOut, Animation<float>::kEaseOut),
  ...
  private:
    ...
    OBSERVE_LEAK_CHECKER(ScrollBar)
  };
  ...
  public:
    ...
    explicit ScrollableFrame(const std::string &name = "") : Frame(name)
    {
      ...
      container_.onChildAdded() = [this](const Frame *child)
      { updateScrollableHeight(child); };
  ...
  private:
    ...
    OBSERVE_LEAK_CHECKER(ScrollableFrame)
  };
}