...
namespace observe
{
  ...
  struct Dimension
  {
    ...
    Dimension(float amount, std::function<float(float, float, float, float)> compute) : amount(amount), compute_function(std::move(compute)) {}

    static Dimension nativePixels(float pixels)
    {
      return Dimension(pixels, [](float amount, float, float, float)
                       { return amount; });
    }

    static Dimension logicalPixels(float pixels)
    {
      return Dimension(pixels, [](float amount, float scale, float, float)
                       { return scale * amount; });
    }
