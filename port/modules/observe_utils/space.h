...
namespace observe
{
  ...
  struct BaseTransform
  {
    ...
    BaseTransform(BaseMatrix<T> matrix, BasePoint<T> translate) : matrix(matrix), translate(translate) {}

    BaseTransform(BaseMatrix<T> matrix) : matrix(matrix) {}

    BaseTransform(T a1, T a2, T a3, T b1, T b2, T b3) : matrix(a1, a2, b1, b2), translate(a3, b3) {}
  ...
  public:
    Bounds() = default;
    Bounds(float x, float y, float width, float height) : x_(x), y_(y), width_(width), height_(height) {}

    explicit Bounds(const IBounds &other) : x_(other.x()), y_(other.y()), width_(other.width()), height_(other.height()) {}
