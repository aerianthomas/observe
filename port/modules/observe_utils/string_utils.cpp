...
namespace observe
{
  ...
  {
    std::u32string result = string_;
    auto filter = [&characters](char32_t c)
    { return characters.find(c) != std::u32string::npos; };
  ...
  {
    std::u32string result = string_;
    auto filter = [](char32_t c)
    { return (c & 0xfffffff0) == 0xfe00; };
    result.erase(std::remove_if(result.begin(), result.end(), filter), result.end());
    return result;
  }
