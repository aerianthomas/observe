...
#include "modules/observe_graphics/graphics_utils.h"
#include "modules/observe_utils/child_process.h"
...
namespace observe
{
                                          ...
                                          const std::function<void(std::string)> &callback)
  {
    ...
    std::string result;
    if (observe::preprocessWebGlShader(result, code, utils, varying))
    {
  ...
  bool ShaderCompiler::compileShader()
  {
    ...
    {
      runOnEventThread([callback]
                       { callback("Shader compiler not found"); });
      return false;
    }
    ...
    if (!spawnChildProcess(compiler_path_, arguments, output))
    {
      ...
      runOnEventThread([callback, output]()
                       { callback(output); });
      return false;
    }
