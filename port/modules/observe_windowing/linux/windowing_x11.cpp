...
#include "observe_utils/thread_utils.h"
...
namespace observe
{
  ...
  std::string readClipboardText()
  {
    ...
    Atom selection_property = XInternAtom(display, "OBSERVE_SELECT", False);
  ...
  void setCursorStyle(MouseCursor style)
  {
    ...
    switch (style)
    {
    case MouseCursor::Invisible:
      cursor = x11->cursors().no_cursor;
      break;
    case MouseCursor::Arrow:
      cursor = x11->cursors().arrow_cursor;
      break;
    case MouseCursor::IBeam:
      cursor = x11->cursors().ibeam_cursor;
      break;
    case MouseCursor::Crosshair:
      cursor = x11->cursors().crosshair_cursor;
      break;
    case MouseCursor::Pointing:
      cursor = x11->cursors().pointing_cursor;
      break;
    case MouseCursor::HorizontalResize:
      cursor = x11->cursors().horizontal_resize_cursor;
      break;
    case MouseCursor::VerticalResize:
      cursor = x11->cursors().vertical_resize_cursor;
      break;
    case MouseCursor::TopLeftResize:
      cursor = x11->cursors().top_left_resize_cursor;
      break;
    case MouseCursor::TopRightResize:
      cursor = x11->cursors().top_right_resize_cursor;
      break;
    case MouseCursor::BottomLeftResize:
      cursor = x11->cursors().bottom_left_resize_cursor;
      break;
    case MouseCursor::BottomRightResize:
      cursor = x11->cursors().bottom_right_resize_cursor;
      break;
  ...
  static MouseCursor windowResizeCursor(int operation)
  {
    switch (operation)
    {
    case kResizeLeft | kResizeTop:
      return MouseCursor::TopLeftResize;
    case kResizeRight | kResizeTop:
      return MouseCursor::TopRightResize;
    case kResizeLeft | kResizeBottom:
      return MouseCursor::BottomLeftResize;
    case kResizeRight | kResizeBottom:
      return MouseCursor::BottomRightResize;
    case kResizeLeft:
    case kResizeRight:
      return MouseCursor::HorizontalResize;
    case kResizeTop:
    case kResizeBottom:
      return MouseCursor::VerticalResize;
    default:
                             ...
                             const std::string &message)
  {
    ...
    if (!font)
    {
      OBSERVE_LOG("Unable to load font");
      return;
    }
  ...
  {
    OBSERVE_ASSERT(bounds.width() && bounds.height());
    ...
    XStoreName(display, window_handle_, OBSERVE_APPLICATION_NAME);
  ...
  WindowX11::WindowX11(int x, int y, int width, int height, Decoration decoration) : Window(width, height), decoration_(decoration)
  {
  ...
  void WindowX11::showMaximized()
  {
    ...
    for (int retries = 0; retries < 10; ++retries)
    {
      ...
      while (XPending(x11_->display()))
      {
        ...
        {
          observe::IPoint dimensions = retrieveWindowDimensions();
          handleResized(dimensions.x, dimensions.y);
          return;
        }
      }
