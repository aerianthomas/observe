...
#include "observe_utils/file_system.h"
#include "observe_utils/time_utils.h"
...
@implementation ObserveDraggingSource
...
@implementation ObserveAppViewDelegate
- (instancetype)initWithWindow:(observe::WindowMac*)window {
  self = [super init];
  self.observe_window = window;
  self.start_microseconds = observe::time::microseconds();
  return self;
}

- (void)drawWindow {
  long long ms = observe::time::microseconds();
  self.observe_window->drawCallback((ms - self.start_microseconds) / 1000000.0);
}

- (void)mtkView:(MTKView*)view drawableSizeWillChange:(CGSize)size {
  self.observe_window->handleNativeResize(size.width, size.height);
}
...
- (void)drawInMTKView:(MTKView*)view {
  ...
  view.layer.contentsScale = self.observe_window->dpiScale();
  [self drawWindow];
}
@end

@implementation ObserveAppView
- (instancetype)initWithFrame:(NSRect)frame_rect inWindow:(observe::WindowMac*)window {
  ...
  self.drag_source = [[ObserveDraggingSource alloc] init];
...
- (void)keyDown:(NSEvent*)event {
  ...
  if (self.allow_quit && command && q_or_w) {
    observe::NativeWindowLookup::instance().closeAll();
    return;
  }

  int modifiers = [self keyboardModifiers:event];
  if ((modifiers & observe::kModifierCmd) == 0)
    [self interpretKeyEvents:[NSArray arrayWithObject:event]];

  observe::KeyCode key_code = observe::translateKeyCode([event keyCode]);
  if (!self.observe_window->handleKeyDown(key_code, modifiers, [event isARepeat]))
    [super keyDown:event];
}

- (void)keyUp:(NSEvent*)event {
  observe::KeyCode key_code = observe::translateKeyCode([event keyCode]);
  if (!self.observe_window->handleKeyUp(key_code, [self keyboardModifiers:event]))
    [super keyUp:event];
}

- (void)insertText:(id)string {
  self.observe_window->handleTextInput([string UTF8String]);
}
...
- (observe::Point)eventPosition:(NSEvent*)event {
  NSPoint location = [event locationInWindow];
  NSPoint view_location = [self convertPoint:location fromView:nil];
  CGFloat view_height = self.frame.size.height;
  return observe::Point(view_location.x, view_height - view_location.y) * self.observe_window->dpiScale();
}

- (observe::Point)dragPosition:(id<NSDraggingInfo>)sender {
  NSPoint drag_point = [self convertPoint:[sender draggingLocation] fromView:nil];
  CGFloat view_height = self.frame.size.height;
  return observe::Point(drag_point.x, view_height - drag_point.y) * self.observe_window->dpiScale();
}
...
- (int)mouseButtonState {
  ...
  int result = 0;
  if (buttons & (1 << 0))
    result = result | observe::kMouseButtonLeft;
  if (buttons & (1 << 1))
    result = result | observe::kMouseButtonRight;
  if (buttons & (1 << 2))
    result = result | observe::kMouseButtonMiddle;
  return result;
}
...
- (int)keyboardModifiers:(NSEvent*)event {
  ...
  if (flags & NSEventModifierFlagCommand)
    result = result | observe::kModifierCmd;
  if (flags & NSEventModifierFlagControl)
    result = result | observe::kModifierMacCtrl;
  if (flags & NSEventModifierFlagOption)
    result = result | observe::kModifierOption;
  if (flags & NSEventModifierFlagShift)
    result = result | observe::kModifierShift;
  return result;
}

- (void)checkRelativeMode {
  if (self.observe_window->mouseRelativeMode()) {
    CGAssociateMouseAndMouseCursorPosition(false);
...
- (void)makeKeyWindow {
  if (!self.observe_window->isPopup())
    [self.window makeKeyWindow];
}

- (void)scrollWheel:(NSEvent*)event {
  static constexpr float kPreciseScrollingScale = 0.008f;
  observe::Point point = [self eventPosition:event];
  ...
  self.observe_window->handleMouseWheel(delta_x, delta_y, precise_x, precise_y, point.x, point.y,
...
- (void)mouseMoved:(NSEvent*)event {
  observe::Point point = [self eventPosition:event];
  self.observe_window->handleMouseMove(point.x, point.y, [self mouseButtonState],
...
- (void)mouseExited:(NSEvent*)event {
  self.observe_window->handleMouseLeave([self mouseButtonState], [self keyboardModifiers:event]);
}

- (void)mouseDown:(NSEvent*)event {
  observe::Point point = [self eventPosition:event];
  self.mouse_down_screen_position = [self mouseScreenPosition];
  self.observe_window->handleMouseDown(observe::kMouseButtonLeft, point.x, point.y,
                                      [self mouseButtonState], [self keyboardModifiers:event]);
  [self makeKeyWindow];
  if (self.observe_window->isDragDropSource()) {
    observe::File file = self.observe_window->startDragDropSource();
...
- (void)mouseUp:(NSEvent*)event {
  observe::Point point = [self eventPosition:event];
  self.observe_window->handleMouseUp(observe::kMouseButtonLeft, point.x, point.y,
                                    [self mouseButtonState], [self keyboardModifiers:event]);
}
...
- (void)rightMouseDown:(NSEvent*)event {
  observe::Point point = [self eventPosition:event];
  self.mouse_down_screen_position = [self mouseScreenPosition];
  self.observe_window->handleMouseDown(observe::kMouseButtonRight, point.x, point.y,
                                      [self mouseButtonState], [self keyboardModifiers:event]);
  [self makeKeyWindow];
}
...
- (void)rightMouseUp:(NSEvent*)event {
  observe::Point point = [self eventPosition:event];
  self.observe_window->handleMouseUp(observe::kMouseButtonRight, point.x, point.y,
                                    [self mouseButtonState], [self keyboardModifiers:event]);
}
...
- (void)otherMouseDown:(NSEvent*)event {
  ...
  observe::Point point = [self eventPosition:event];
  self.mouse_down_screen_position = [self mouseScreenPosition];
  self.observe_window->handleMouseDown(observe::kMouseButtonMiddle, point.x, point.y,
                                      [self mouseButtonState], [self keyboardModifiers:event]);
  [self makeKeyWindow];
}
...
- (void)otherMouseUp:(NSEvent*)event {
  ...
  observe::Point point = [self eventPosition:event];
  self.observe_window->handleMouseUp(observe::kMouseButtonMiddle, point.x, point.y,
                                    [self mouseButtonState], [self keyboardModifiers:event]);
}
...
- (void)viewDidChangeBackingProperties {
  ...
  if (self.observe_window)
    self.observe_window->resetBackingScale();
}
...
- (void)viewWillMoveToWindow:(NSWindow*)new_window {
  ...
  if (new_window) {
    ...
    if (self.observe_window)
      self.observe_window->setParentWindow(new_window);
  ...
  else if (self.observe_window)
    self.observe_window->closeWindow();
}

- (void)windowOcclusionChanged:(NSNotification*)notification {
  NSWindow* window = notification.object;
  if (self.observe_window)
    self.observe_window->setVisible(window.occlusionState & NSWindowOcclusionStateVisible);
}
...
- (NSDragOperation)dragFiles:(id<NSDraggingInfo>)sender {
  observe::Point drag_point = [self dragPosition:sender];
  std::vector<std::string> files = [self dropFileList:sender];
  if (self.observe_window->handleFileDrag(drag_point.x, drag_point.y, files))
    return NSDragOperationCopy;
...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender {
  observe::Point drag_point = [self dragPosition:sender];
  std::vector<std::string> files = [self dropFileList:sender];
  if (self.observe_window->handleFileDrop(drag_point.x, drag_point.y, files))
    return NSDragOperationCopy;
  return NSDragOperationNone;
}
...
- (void)draggingExited:(id<NSDraggingInfo>)sender {
  self.observe_window->handleFileDragLeave();
}

@end

@implementation ObserveAppWindowDelegate
- (BOOL)windowShouldClose:(NSWindow*)sender {
  NSView* view = [sender contentView];
  observe::WindowMac* window = observe::NativeWindowLookup::instance().findWindow((__bridge void*)view);
...
- (NSSize)windowWillResize:(NSWindow*)sender toSize:(NSSize)frame_size {
  ...
  observe::Point borders = observe::windowBorderSize(self.window_handle);
  float scale = self.observe_window->dpiScale();
  int width = scale * (frame_size.width - borders.x);
  int height = scale * (frame_size.height - borders.y);
  self.observe_window->handleAdjustResize(&width, &height, self.resizing_horizontal, self.resizing_vertical);
  return NSMakeSize(width / scale + borders.x, height / scale + borders.y);
}

@end

@implementation ObserveAppDelegate
- (void)applicationDidFinishLaunching:(NSNotification*)notification {
  self.window_delegate = [[ObserveAppWindowDelegate alloc] init];
  self.window_delegate.observe_window = self.observe_window;
...
- (NSApplicationTerminateReply)applicationShouldTerminate:(NSApplication*)sender {
  if (observe::NativeWindowLookup::instance().requestAllToClose())
...
namespace observe {
  ...
  void WindowMac::runEventLoop() {
    ...
    @autoreleasepool {
      ...
      ObserveAppDelegate* delegate = [[ObserveAppDelegate alloc] init];
      delegate.observe_window = this;
  ...
  WindowMac::WindowMac(int x, int y, int width, int height, float scale, Decoration decoration) :
    ...
    rect.origin.x = 0;
    rect.origin.y = 0;
    view_ = [[ObserveAppView alloc] initWithFrame:rect inWindow:this];
    view_delegate_ = [[ObserveAppViewDelegate alloc] initWithWindow:this];
  ...
  WindowMac::WindowMac(int width, int height, float scale, void* parent_handle) :
    ...
    view_ = [[ObserveAppView alloc] initWithFrame:view_frame inWindow:this];
    view_delegate_ = [[ObserveAppViewDelegate alloc] initWithWindow:this];
    view_.delegate = view_delegate_;
