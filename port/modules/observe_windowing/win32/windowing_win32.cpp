...
#include "modules/observe_ui/win32/menu_bar.h"
...
namespace observe
{
  ...
  class DpiAwarenessMatchingWindow
  {
  public:
    explicit DpiAwarenessMatchingWindow(HWND hwnd)
    {
      if (hwnd == nullptr)
        return;
      ...
      setThreadDpiAwarenessContext_ =
          procedure<SetThreadDpiAwarenessContext_t>(user32, "SetThreadDpiAwarenessContext");
      if (windowDpiAwarenessContext == nullptr || threadDpiAwarenessContext_ == nullptr ||
          setThreadDpiAwarenessContext_ == nullptr)
      {
        return;
      }

      previous_dpi_awareness_ = threadDpiAwarenessContext_();
      DPI_AWARENESS_CONTEXT target = windowDpiAwarenessContext(hwnd);
      if (target)
        changed_ = setThreadDpiAwarenessContext_(target) != nullptr;
    }

    ~DpiAwarenessMatchingWindow()
    {
      if (changed_ && setThreadDpiAwarenessContext_)
        setThreadDpiAwarenessContext_(previous_dpi_awareness_);
    }
  ...
  { // TODO check if mouse down is touch
...
#if OBSERVE_WINDOWS_ICON_RESOURCE
  ...
  static float dpiScaleForPluginParent(HWND parent_handle)
  {
    // Hosted plugin HWNDs must use the host parent's coordinate space. Ableton's
    // Auto-Scale path can make the child report 96 DPI even on a 200% monitor.
    DpiAwareness dpi_awareness;
    if (parent_handle)
      return dpi_awareness.dpiScale(parent_handle);
  ...
  std::unique_ptr<Window> createPluginWindow(const Dimension &width, const Dimension &height,
                                             void *parent_handle)
  {
    auto parent_hwnd = static_cast<HWND>(parent_handle);
    float dpi_scale = dpiScaleForPluginParent(parent_hwnd);
    HMONITOR monitor = MonitorFromWindow(parent_hwnd, MONITOR_DEFAULTTONEAREST);
    IBounds bounds = boundsInMonitor(monitor, dpi_scale, {}, {}, width, height);
    return std::make_unique<WindowWin32>(bounds.width(), bounds.height(), parent_handle);
  }

  WindowWin32::WindowWin32(int x, int y, int width, int height, Decoration decoration) : Window(width, height), decoration_(decoration)
  {
    ...
    {
      OBSERVE_LOG("Error creating window");
      return;
    }
  ...
  WindowWin32::WindowWin32(int width, int height, void *parent_handle) : Window(width, height)
  {
    ...
    setDpiScale(dpiScaleForPluginParent(parent_handle_));

    std::string app_name = OBSERVE_APPLICATION_NAME;
    // Ableton Auto-Scale hosts the plugin in a DPI-virtualized wrapper. Create
    // the child while the thread matches the wrapper so HWND and DXGI sizes agree.
    DpiAwarenessMatchingWindow dpi_scope(parent_handle_);
    window_handle_ = CreateWindow(window_class_.lpszClassName,
  ...
  void WindowWin32::finishWindowSetup()
  {
    ...
    attachNativeMenuBar(window_handle_);
  ...
  WindowWin32::~WindowWin32()
  {
    ...
    detachNativeMenuBar(window_handle_);
  ...
  void WindowWin32::handleResizeEnd(HWND hwnd)
  {
    ...
    if (parent_handle_)
    {
      // Keep plugin children in the host parent's DPI space instead of letting
      // Ableton's DPI-unaware child HWND flip the scale during resize.
      setDpiScale(dpiScaleForPluginParent(parent_handle_));
    }
    else
    {
  ...
  void WindowWin32::handleDpiChange(HWND hwnd, LPARAM l_param, WPARAM w_param)
  {
    ...
    if (parent_handle_)
    {
      // WM_DPICHANGED on plugin views follows the host wrapper, not an
      // independent top-level observe window.
      setDpiScale(dpiScaleForPluginParent(parent_handle_));
    }
    else
    {
